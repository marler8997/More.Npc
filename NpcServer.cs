...
namespace More
{
    ...
    public class NpcServerSingleThreaded : IDisposable
    {
        ...
        readonly SelectServerSharedBuffer selectServer;
        ...
        public NpcServerSingleThreaded(INpcServerCallback callback, NpcExecutor npcExecutor, INpcHtmlGenerator htmlGenerator, UInt16 port)
        {
            ...
            selectServer = new SelectServerSharedBuffer(false, new Byte[SingleThreadedReceiveBufferLength]);
            selectServer.AddListenSocket(listenSocket, AcceptCallback);
        }
        ...
        public void Dispose()
        {
            selectServer.ShutdownAndCloseSockets();
        }
        public ThreadStart Run { get { return selectServer.Run; } }

        public void AcceptCallback(SelectServerSharedBuffer server, Socket listenSocket)
        {
            Socket clientSocket = listenSocket.Accept();
            if (clientSocket.Connected)
            {
                ...
                server.AddReceiveSocket(clientSocket, dataHandler.InitialRecvHandler);
            }
            else
            {
                clientSocket.Close();
