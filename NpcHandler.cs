...
namespace More
{
    ...
    public class NpcSocketHandler : NpcHandler
    {
            ...
            : base(clientString, callback, npcExecutor, npcHtmlGenerator)
        {
            ...
            this.lineParser = new LineParser(Encoding.ASCII, 256);
        }

        // Returns false on error
        public Boolean TryReceive(Socket socket, byte[] safeBuffer)
        {
            int bytesReceived;
            try
            {
                bytesReceived = socket.Receive(safeBuffer);
            }
            ...
            lineParser.Add(safeBuffer, 0, (uint)bytesReceived);
            return true; // Success
        }


        void HandleNpcLines(SelectServerSharedBuffer server, Socket socket, ByteBuilder builder, String line)
        {
            do
            {
                if (!HandleLine(builder, line))
                {
                    server.ShutdownDisposeAndRemoveReceiveSocket(socket);
                    break;
                }
                // Update the buffer with the bigger buffer
                if (builder.bytes != server.sharedBuffer)
                {
                    server.sharedBuffer = builder.bytes;
                }
        ...
        public void InitialRecvHandler(SelectServerSharedBuffer server, Socket socket)
        {
            try
            {
                if (!TryReceive(socket, server.sharedBuffer))
                {
                    server.ShutdownDisposeAndRemoveReceiveSocket(socket);
                    return;
                }
                ...
                ByteBuilder builder = new ByteBuilder(server.sharedBuffer);
                ...
                server.UpdateHandler(socket, AfterFirstLineRecvHandler);
                HandleNpcLines(server, socket, builder, line);
            }
            catch (Exception e)
            {
        ...
        public void AfterFirstLineRecvHandler(SelectServerSharedBuffer server, Socket socket)
        {
            try
            {
                if (!TryReceive(socket, server.sharedBuffer))
    ...
    public class NpcBlockingThreadHander : NpcHandler
    {
        ...
        {
            this.socket = socket;
            this.socketLineReader = new SocketLineReader(socket, Encoding.ASCII, 256);
        }
        public void Run()
        {
            try
